...
#include <face_recognition/coordenadas.h>
...
public:
    ...
    as_(nh_, name, boost::bind(&FaceRecognition::executeCB, this, _1), false)
  {
    ...
    image_sub_ = it_.subscribe("/roah_ipcam/image", 1, &FaceRecognition::imageCB, this);
  }
  ...
  void imageCB(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    //Aqui estan los parametros del ROI
    ros::NodeHandle n;
    ros::Publisher chatter_pub = n.advertise<face_recognition::coordenadas>("chatter", 1000);
    ros::Rate loop_rate(10);
    face_recognition::coordenadas coor;
    coor.x = faceRect.x;
    coor.y = faceRect.y;
    coor.w = faceRect.width;
    coor.h = faceRect.height;
    chatter_pub.publish(coor);
    ros::spinOnce();
    loop_rate.sleep();
    //Aqui termina de mandar los datos del ROI
     ...
     if( goal_id_<2 )      
    {
	 ...
	 ROS_WARN("Alert: Database is not updated. Please delete \"facedata.xml\" and re-run!"); //AFTER
       ...
       if(confidence<confidence_value)
       {
          ...
          //Aqui publicar que no es el doctor
       }
       else
       {
          text_image <<  frl.personNames[nearest-1].c_str()<<" is recognized";
          cvPutText(img, text_image.str().c_str(), cvPoint(faceRect.x, faceRect.y + faceRect.height + 25), &font, textColor);
          //Aqui publicar que es el Doctor
